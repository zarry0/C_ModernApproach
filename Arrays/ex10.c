...
int main (void) {
    ...
    const int departuresLen = (int) sizeof(departureMidnight) / sizeof(departureMidnight[0]); 

    int index = 0;
    while (true) {
        int a = abs(minutesSinceMidnight - departureMidnight[index]);
        int b = abs(minutesSinceMidnight - departureMidnight[index+1]);
        if (a < b)
            break; 
        index++;
    }
